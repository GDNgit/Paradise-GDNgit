...
//TODO: convert ALL of these to spells
/mob/living/carbon/xenomorph/larva/verb/hide()
	...
	if(layer != ABOVE_NORMAL_TURF_LAYER)
		...
		visible_message("<B>[src] scurries to the ground!</B>", "<span class='noticexenomorph'>You are now hiding.</span>")
	else
		layer = MOB_LAYER
...
/mob/living/carbon/xenomorph/larva/verb/evolve()
	set name = "Evolve"
	set desc = "Evolve into a fully grown Xenomorph."
	set category = "Xenomorph"
	...
	if(amount_grown >= max_grown)	//TODO ~Carn
		...
		to_chat(src, "<span class='boldnotice'>You are growing into a beautiful xenomorph! It is time to choose a caste.</span>")
		...
		to_chat(src, "<B>Drones</B> <span class='notice'>are the working class, offering the largest plasma storage and generation. They are the only caste which may evolve again, turning into the dreaded xenomorph queen.</span>")
		var/xenomorph_caste = alert(src, "Please choose which xenomorph caste you shall belong to.",,"Hunter","Sentinel","Drone")
