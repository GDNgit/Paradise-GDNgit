...
/mob/living/carbon/xenomorph/proc/get_caste_organs()
...
/mob/living/carbon/xenomorph/get_default_language()
	if(default_language)
		return default_language
	return GLOB.all_languages["Xenomorph"]

/mob/living/carbon/xenomorph/say_quote(message, datum/language/speaking = null)
	var/verb = "hisses"
...
/mob/living/carbon/xenomorph/check_eye_prot()
	return 2
...
//	to_chat(world, "Loc temp: [loc_temp] - Body temp: [bodytemperature] - Fireloss: [getFireLoss()] - Fire protection: [heat_protection] - Location: [loc] - src: [src]")
	...
	// Xenomorphs are rather weak to fire
	...
	if(bodytemperature > 360.15)
		...
		throw_alert("xenomorph_fire", /obj/screen/alert/xenomorph_fire)
...
/mob/living/carbon/xenomorph/SetStunned(amount, updating = TRUE, force = 0)
	..()
...
/mob/living/carbon/xenomorph/movement_delay()
	. = ..()
	. += move_delay_add + GLOB.configuration.movement.xenomorph_delay //move_delay_add is used to slow xenomorphs with stuns
...
/mob/living/carbon/xenomorph/verb/nightvisiontoggle()
...
/mob/living/carbon/xenomorph/assess_threat(mob/living/simple_animal/bot/secbot/judgebot, lasercolor)
	if(judgebot.emagged == 2)
		return 10 //Everyone is a criminal!
	var/threatcount = 0

	//Securitrons can't identify xenomorphs
...
Des: Gives the client of the xenomorph an image on each infected mob.
----------------------------------------*/
/mob/living/carbon/xenomorph/proc/AddInfectionImages()
	if(client)
		for(var/mob/living/C in GLOB.mob_list)
			if(HAS_TRAIT(C, TRAIT_XENO_HOST))
				var/obj/item/organ/internal/body_egg/xenomorph_embryo/A = C.get_int_organ(/obj/item/organ/internal/body_egg/xenomorph_embryo)
				if(A)
					var/I = image('icons/mob/xenomorph.dmi', loc = C, icon_state = "infected[A.stage]")
					client.images += I
	return
...
Des: Removes all infected images from the xenomorph.
----------------------------------------*/
...
/mob/living/carbon/xenomorph/canBeHandcuffed()
	return TRUE

/mob/living/carbon/xenomorph/proc/updatePlasmaDisplay()
	if(hud_used) //clientless xenomorphs
		hud_used.xenomorph_plasma_display.maptext = "<div align='center' valign='middle' style='position:relative; top:0px; left:6px'> <font face='Small Fonts' color='magenta'>[getPlasma()]</font></div>"
		hud_used.xenomorph_plasma_display.maptext_x = -3
...
/mob/living/carbon/xenomorph/on_lying_down(new_lying_angle)
	. = ..()
