...
// replaced shitcode with shitcode feels good
/obj/item/organ/internal/body_egg/xenomorph_embryo
	name = "xenomorph embryo"
...
/obj/item/organ/internal/body_egg/xenomorph_embryo/on_find(mob/living/finder)
	..()
	if(stage < 4)
...
/obj/item/organ/internal/body_egg/xenomorph_embryo/prepare_eat()
	var/obj/S = ..()
	S.reagents.add_reagent("sacid", 10)
	return S

/obj/item/organ/internal/body_egg/xenomorph_embryo/on_life()
	switch(stage)
		if(2)
			if(prob(2))
				owner.emote("sneeze")
...
/obj/item/organ/internal/body_egg/xenomorph_embryo/egg_process()
...
/obj/item/organ/internal/body_egg/xenomorph_embryo/proc/AttemptGrow(burst_on_success = TRUE)
	if(!owner || polling)
		return
	polling = TRUE
	spawn()
		var/list/candidates = SSghost_spawns.poll_candidates("Do you want to play as an xenomorph?", ROLE_XENOMORPH, FALSE, source = /mob/living/carbon/xenomorph/larva)
		var/mob/C = null

		// To stop clientless larva, we will check that our host has a client
		// if we find no ghosts to become the xenomorph. If the host has a client
		// he will become the xenomorph but if he doesn't then we will set the stage
		...
		var/overlay = image('icons/mob/xenomorph.dmi', loc = owner, icon_state = "burst_lie")
		owner.add_overlay(overlay)

		spawn(6)
			owner.cut_overlay(overlay)
			var/mob/living/carbon/xenomorph/larva/new_xeno = new(owner.drop_location())
			new_xeno.key = C.key
...
Des: Adds the infection image to all xenomorphs for this embryo
----------------------------------------*/
/obj/item/organ/internal/body_egg/xenomorph_embryo/AddInfectionImages()
	for(var/mob/living/carbon/xenomorph/xenomorph in GLOB.player_list)
		if(xenomorph.client)
			var/I = image('icons/mob/xenomorph.dmi', loc = owner, icon_state = "infected[stage]")
			xenomorph.client.images += I

/*----------------------------------------
Proc: RemoveInfectionImage(C)
Des: Removes all images from the mob infected by this embryo
----------------------------------------*/
/obj/item/organ/internal/body_egg/xenomorph_embryo/RemoveInfectionImages()
	for(var/mob/living/carbon/xenomorph/xenomorph in GLOB.player_list)
		if(xenomorph.client)
			for(var/image/I in xenomorph.client.images)
