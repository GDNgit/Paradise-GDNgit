...
/obj/item/clothing/mask/facehugger
	...
	w_class = WEIGHT_CLASS_TINY //note: can be picked up by xenomorphs unlike most other items of w_class below 4
	throw_range = 5
	tint = 3
	flags = AIRTIGHT
...
/obj/item/clothing/mask/facehugger/attack_xenomorph(mob/user) //can be picked up by xenomorphs
...
/obj/item/clothing/mask/facehugger/proc/Attach(mob/living/M)
	if(!isliving(M))
		return FALSE
	if((!iscorgi(M) && !iscarbon(M)) || isxenomorph(M))
		return FALSE
	if(attached)
		return FALSE
	else
...
/obj/item/clothing/mask/facehugger/proc/Impregnate(mob/living/target)
	...
	if(!sterile)
		//target.contract_disease(new /datum/disease/xenomorph_embryo(0)) //so infection chance is same as virus infection chance
		target.visible_message("<span class='danger'>[src] falls limp after violating [target]'s face!</span>", \
								"<span class='userdanger'>[src] falls limp after violating [target]'s face!</span>")
		Die()
		icon_state = "[initial(icon_state)]_impregnated"

		if(!target.get_int_organ(/obj/item/organ/internal/body_egg/xenomorph_embryo))
			new /obj/item/organ/internal/body_egg/xenomorph_embryo(target)
	else
