...
/mob/living/carbon/xenomorph/humanoid/update_icons()
	...
	if(stat == DEAD)
		...
		if(getFireLoss() > 125)
			icon_state = "xenomorph[caste]_husked"
			pixel_y = 0
		else
			icon_state = "xenomorph[caste]_dead"
			pixel_y = 0

	else if(stat == UNCONSCIOUS || IsWeakened())
		icon_state = "xenomorph[caste]_unconscious"
		pixel_y = 0
	else if(leap_on_click)
		icon_state = "xenomorph[caste]_pounce"

	else if(IS_HORIZONTAL(src))
		icon_state = "xenomorph[caste]_sleep"
	else if(m_intent == MOVE_INTENT_RUN)
		icon_state = "xenomorph[caste]_running"
	else
		icon_state = "xenomorph[caste]_s"
	...
	if(leaping)
		if(alt_icon == initial(alt_icon))
			...
			icon = alt_icon
			alt_icon = old_icon
		icon_state = "xenomorph[caste]_leap"
		pixel_x = -32
		pixel_y = -32
	else
		if(alt_icon != initial(alt_icon))
...
/mob/living/carbon/xenomorph/humanoid/update_transform() //The old method of updating lying/standing was update_icons(). xenomorphs still expect that.
	..()
	update_icons()
...
/mob/living/carbon/xenomorph/humanoid/update_inv_head()
	if(head)
		...
		head.screen_loc = ui_xenomorph_head
